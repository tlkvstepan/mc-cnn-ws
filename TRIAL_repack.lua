...
tmp = torch.load('work/RETRAIN-fst-kitti_ext/metricNet_RETRAIN-fst-kitti_ext.t7', 'ascii')
embed_net = tmp[1].modules[1].modules[1]

head_net:cuda()
embed_net:cuda()

--cudnn.convert(head_net, cudnn)
--cudnn.convert(embed_net, cudnn)

tmp = {}
tmp[1] = embed_net
tmp[2] = head_net
 torch.save('metricNet_RETRAIN-fst-kitti_ext.t7', tmp, 'ascii');
