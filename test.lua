...
function stereo_predict(im0, im1, disc0, disc1, prm) 
    
  local vols = torch.CudaTensor(2, disp_max, input:size(3), input:size(4)):fill(0 / 0)
  adcensus.StereoJoin(disc0, disc1, vols[{{1}}], vols[{{2}}])
  ...
  for _, direction in ipairs({1, -1}) do
    ...
    do
      ...
      out:zero()
      adcensus.sgm2(im0, im1, vol, out, tmp, prm.pi1, prm.pi2, prm.tau_so, prm.alpha1, prm.sgm_q1, prm.sgm_q2, direction)
      vol:copy(out):div(4)
  ...
  disp[2] = adcensus.mean2d(disp[2], gaussian(prm.blur_sigma):cuda(), prm.blur_t)
  outlier = nil
  disp[1] = nil
...
-- load feature net
net0 = torch.load('work/contrast-dprog/fnet_2016_10_15_18:25:25_contrast-dprog.t7', 'ascii')

-------------------------------
-- make feature nets
for i = 1,#net_te.modules do
  if torch.typename(net0.modules[i]) == 'nn.SpatialConvolution' then
    net0.modules[i].padW = 1
    net0.modules[i].padH = 1
  end
end
net0:add(nn.Normalize2())
net0:cuda()
cudnn.convert(net0, cudnn)      
net1 = net0:clone('weight','bias', 'gradWeight','gradBias');

---------------------------------
-- pass input through feature nets
net0:forward(
...
disc0 = net:forward(input[1])
disc1 = net:forward(input[1])
