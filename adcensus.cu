...
// Remove disparities disp such that (x-disp) < 0 
__global__ void remove_nonvisible(float *disp, int nb_elem, int width)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < nb_elem) {
		int x = id % width;
...
// Removes disparities that correspond to occluded patches, such that they are
// overshadowed on right image
__global__ void remove_occluded(float *disp, int nb_elem, int width)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < nb_elem) {
		int x = id % width;
		for (int dx = 1; x + dx < width; dx++) {
			if (dx - disp[id + dx] < -disp[id]) {
				disp[id] = 0;
				break;
			}
		}
...
// Removes disparity that corresponds to saturated pixels
__global__ void remove_white(float *im, float *disp, int nb_elem)
{
	int id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < nb_elem) {
		if (im[id] == 255) {
