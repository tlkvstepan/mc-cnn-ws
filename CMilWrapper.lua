...
function milWrapper.getMilDprogContrastDprogDoubleBatch(img_w, disp_max, hpatch, dist_min, fnet)
...
function milWrapper.getDynProgNetDoubleBatch(img_w, disp_max, hpatch, dist_min, fnet)  
...
-- stream1 :
stream1:add(nn.Narrow(1, disp_max+1, img_w - 2*hpatch - disp_max))
stream1:add(nn.contrastDynProgMax(dist_min))

-- stream2 :
stream2:add(nn.Narrow(2,1, img_w - 2*hpatch - disp_max))
stream2:add(nn.Transpose({1,2}))
stream2:add(nn.contrastDynProgMax(dist_min))
