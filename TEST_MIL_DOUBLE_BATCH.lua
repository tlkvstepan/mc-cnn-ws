...
-- |parameteres|
-- learning
-- 1000 * 32 * 10 = 320000 one epoch
-- 600e6 in total
local test_set_size = 50000;     -- 50000
local batch_size = 32            -- 32 * 1000 = 32000
local epoch_size = batch_size*10 -- 10
local nb_epoch = 600;
-- loss
local margin = 0.2;
-- network
...
nbConvLayers = 5;
-- debug
local suffix = 'largeScale_'
local gpu_on = true;
local debug = true;

-- |read data| from all KITTI
local img1_arr = torch.cat({torch.squeeze(utils.fromfile('data/KITTI12/x0.bin')):double(),
                            torch.squeeze(utils.fromfile('data/KITTI15/x0.bin')):double()},1);
      
local img2_arr = torch.cat({torch.squeeze(utils.fromfile('data/KITTI12/x1.bin')):double(),
                            torch.squeeze(utils.fromfile('data/KITTI15/x1.bin')):double()},1);
    
local disp_arr = torch.round(torch.squeeze(utils.fromfile('data/KITTI12/dispnoc.bin'))):double();
...
local testSet = dl.sup3PatchSet(img1_arr[{{1,50},{},{}}], img2_arr[{{1,50},{},{}}], disp_arr[{{1,50},{},{}}], hpatch);
testSet:shuffle();
...
ftest = function()
    ...
    return acc, acc_le2, acc_le5, err_index    
end
...
for nepoch = 1, nb_epoch do
  ...
  for k, input  in trainSet:sampleiter(batch_size, epoch_size) do
    ...
    optim.adam(feval, _TR_PPARAM_, config)    
    table.insert(sample_err, _TR_ERR_)
            
  end
  
  -- validation
  local test_acc, test_acc_le2, test_acc_le5, err_index  = ftest()
  local train_err = torch.Tensor(sample_err):mean();
  
  -- save failures
  local fail_img = testSet:vis_3_patch(_TE_INPUT_[1]:double(), _TE_INPUT_[2]:double(), _TE_INPUT_[3]:double(), err_index)
  
  if debug then
    image.save('work/'..suffix..'failure.jpg', torch.squeeze(fail_img))
  end
